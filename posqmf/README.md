...
`LogInequalities.lean`, `GtildePositivity.lean` and the `QExpansion` half of `F.lean` depend only on Lean's standard axioms; in particular the Ramanujan axioms of `DifferentialOperators/Ramanujan.lean` are not used there. The `PolynomialModel` half does use them, since the $q$-expansion map bridging the two layers is what turns $DE_2, DE_4, DE_6$ into polynomials in $E_2, E_4, E_6$.
