...
- `F.lean` proves, with nothing assumed, that the Fourier coefficients of $\widetilde{F}_{w-2}$ are positive (Proposition 4.5). It has two sections, one per layer, because `KanekoZagier` and `QuasiModular` both name $D$, $E_2, E_4, E_6$ and $\partial_k$ and so cannot be opened together.
  - `QExpansion` works in $\mathbb{R}[[q]]$: the specialized coefficient formulas, all sign analysis, the boundary estimate with its quartic, the base case $\widetilde{F}_{10} = \frac{1}{360}E_4X_{6,1}$, and the induction. Its conclusions take the recurrence (Lemma 4.4) and the vanishing order and normalization of $F_w$ as explicit hypotheses.
  - `PolynomialModel` discharges all of them, which needs $\delta = \partial/\partial E_2$. The family $F_{4N+12}$ is *defined* there by its recurrence, so $\widetilde{F}_{w-2} := \delta F_w$ is a definition rather than a hypothesis; the base case is $F_{12} = \frac{1}{57600}(E_4')^2$; and the normalization needs the modular linear differential equation $L_{3,w-2}^{((w-4)/4,\,0)}F_w = 0$, checked on $F_{12}$ and propagated along the recurrence with the intertwining lemma.
- `G.lean` proves nonnegativity of the Fourier coefficients of $\widetilde{G}_w$ (Propositions 4.21 and 4.22), including the $A_w$/$B_w$ kernel expansions and the elimination of the boundary coefficient through the third-order equation, plus strict positivity of the constant term. The third-order equation is proved by the intertwining criterion from a check on the constant $\widetilde{G}_0$, and the recurrence is definitional, so the conclusions assume nothing. Unlike `F.lean` this needs no polynomial model, since no $\delta$ appears.

Both files state their induction twice: once taking the recurrence and the differential equation as explicit hypotheses, and once with those discharged. The hypothesis-taking halves (`ftildePos`, `ftildeBoundary`, `gtildePos`, `gtildeConstant_pos`), together with `LogInequalities.lean`, depend only on Lean's standard axioms. The discharged halves use the Ramanujan axioms of `DifferentialOperators/Ramanujan.lean` — in `F.lean` through the $q$-expansion map bridging the two layers, in `G.lean` through the intertwining lemma.

What neither file formalizes is the identification of the generated families with the paper's $F_w$ and $\widetilde{G}_w$. For $F$ the vanishing order, normalization and modular linear differential equation are all proved, which is what characterizes the extremal quasimodular form; for $\widetilde{G}$ the recurrence is taken as the definition, and its derivation from the uniqueness of $G_w = \widetilde{G}_{w-12} \Delta \mathcal{L}_S + \Psi_w$ — an argument about modular functions for $\Gamma(2)$ — remains outside the development.
