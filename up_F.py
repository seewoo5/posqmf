...
# Optional bootstrap values loaded from checkpoint:
# weight -> polynomial dict representation of pos_eigenform(weight)
_POS_EIGENFORM_CHECKPOINT = {}


def _serialize_poly_dict(poly):
    """Serialize polynomial dict into JSON-safe rows."""
    return [
        [e2, e4, e6, coeff.numerator, coeff.denominator]
        for (e2, e4, e6), coeff in sorted(poly.items())
    ]


def _deserialize_poly_dict(rows):
    """Deserialize JSON rows into polynomial dict."""
    poly = {}
    for row in rows:
        if not isinstance(row, list) or len(row) != 5:
            raise ValueError("invalid polynomial row in checkpoint")
        e2, e4, e6, num, den = row
        coeff = _F(int(num), int(den))
        if coeff:
            poly[(int(e2), int(e4), int(e6))] = coeff
    return poly


def _load_checkpoint(path):
    """Load checkpoint payload; returns normalized dict."""
    default = {"version": _CHECKPOINT_VERSION, "last_completed_w": 8, "recent": []}
    ...
    try:
        ...
        print(f"[checkpoint] warning: cannot read {path}: {exc}")
        return default
    ...
    try:
        ...
        last_completed_w = int(payload.get("last_completed_w", 8))
        if last_completed_w % 4 != 0:
            raise ValueError("last_completed_w must be divisible by 4")
        ...
        for entry in raw_recent:
            ...
            if w < 8 or w % 4 != 0:
                continue
            fw = _deserialize_poly_dict(entry.get("F_w", []))
            if not fw:
                continue
            recent.append({"w": w, "F_w": fw})
        recent = sorted(recent, key=lambda x: x["w"])[-_CHECKPOINT_KEEP:]
        return {
            "version": _CHECKPOINT_VERSION,
            "last_completed_w": last_completed_w,
            "recent": recent,
        }
    except Exception as exc:
        print(f"[checkpoint] warning: invalid checkpoint {path}: {exc}")
        return default


def _save_checkpoint(path, last_completed_w, recent):
    ...
    payload = {
        "version": _CHECKPOINT_VERSION,
        "last_completed_w": int(last_completed_w),
        "recent": [
            {
                "w": int(entry["w"]),
                "F_w": _serialize_poly_dict(entry["F_w"]),
            }
            for entry in recent[-_CHECKPOINT_KEEP:]
        ],
    }
    tmp_path = f"{path}.tmp"
...
def pos_eigenform(weight):
    ...
    w = weight
    cached = _POS_EIGENFORM_CHECKPOINT.get(w)
    if cached is not None:
        return cached
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        help="path to checkpoint file (stores most recent F(w) values for resume)",
    )
    ...
    checkpoint_path = args.checkpoint if args.checkpoint else f"{log_path}.checkpoint.json"

    checkpoint = _load_checkpoint(checkpoint_path)
    recent_data = checkpoint["recent"][-_CHECKPOINT_KEEP:]
    for entry in recent_data:
        _POS_EIGENFORM_CHECKPOINT[entry["w"]] = entry["F_w"]

    last_completed_w = checkpoint["last_completed_w"]
    start_w = max(12, last_completed_w + 4)
    log_mode = "a" if start_w > 12 else "w"
    ...
    with open(log_path, log_mode) as f:
        if start_w > 12:
            print(
                ...
                f"last_completed_w={last_completed_w}, restarting at w={start_w}",
                file=f,
                flush=True,
            )

        for w in range(start_w, max_w + 1, 4):
            ds = (4 * w - 64, 4 * w - 40)
            ft = Ftilde(w - 2)
            num_coeffs = w // 4 - 2
            if num_coeffs <= 0:
                continue
            prec = num_coeffs + 5
            coeffs = _eval_qexp(ft, prec)
            all_nonneg = all(coeffs[n] >= 0 for n in range(num_coeffs))
            status = "PASS" if all_nonneg else "FAIL"
            if w % 100 == 0:
                print(f"w={w:5d}, d={ds}: first {num_coeffs} coeffs of Ftilde({w - 2}) nonneg: {status}", file=f, flush=True)

            recent_data.append({"w": w, "F_w": pos_eigenform(w)})
            recent_data = recent_data[-_CHECKPOINT_KEEP:]
            _save_checkpoint(checkpoint_path, w, recent_data)
